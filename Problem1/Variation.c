...
void executeProgramHeap(){
    ...
    generateHeap();
    setIndexHeap();
    calculations -> alreadyIdentified = (int*) (malloc(H * sizeof(int)));
    calculations = executeTreeNodeHiddenIdentify(*(HEAP + 0), readPipe, writePipe, calculations);
}
void executeLeftTree(){
    ...
    calculations -> alreadyIdentified = (int*) (calloc(L + H, sizeof(int)));
    calculations = executeTreeNodeHiddenIdentify(root, readPipe, writePipe, calculations);
}
